                        ...
                        <div class="w-full lg:w-4/6 2xl:w-4/5 lg:mr-5 mb-5">
                            <div class="bg-white overflow-hidden rounded-lg mb-5 lg:hidden">
                                <div class="px-4 py-5 sm:p-6 text-center">
                                    ...
                                    <h4 class="font-bold text-4xl">127 : 42</h4>
                                </div>
                            </div>
                            ...
                            <div id="questionContainer" class="px-4 py-5 sm:p-6 bg-white overflow-hidden shadow-lg ring-1 ring-gray-100 rounded-lg">
                                <div class="flex justify-between items-center">
                                    <h3 class="text-lg font-medium"> Soal No. <span id="number-question"></span> <span class="inline-flex items-center px-2.5 py-0.5 ml-2 rounded-md text-sm font-bold bg-indigo-200 text-indigo-800"> Kompetensi Teknis </span>
                                    </h3>
                        ...
                        <div class="w-full lg:w-2/6 2xl:w-1/5 flex flex-col">
                                    ...
                                    <div>
                                        @if ($tryout?->questions?->count())
                                            @foreach ($tryout?->questions as $key => $question)
                                                <button @if($key == 0) disabled @endif class="bg-gray-200 hover:bg-gray-50 focus:outline-none focus:ring focus:ring-blue-300 py-1.5 rounded m-1 text-center w-9 font-medium @if($key == 0) bg-blue-300 @else bg-gray-200 @endif  text-white">
                                                    {{ $loop->iteration }}
                                                </button>
                                            @endforeach
                                        @endif
                            ...
                            <div class="bg-white overflow-hidden shadow-lg ring-1 ring-gray-100 rounded-lg mb-5 hidden lg:block">
                                <div class="px-4 py-5 sm:p-6 text-center">
                                    ...
                                    <div class="flex justify-center">
                                        <button type="button" class="flex items-center mx-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-red-700 bg-red-200 hover:bg-red-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">BATAL</button>
                                        <button type="button" class="flex items-center mx-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-indigo-500 hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">SELESAI</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div class="mt-2">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-indigo-300 shadow-sm px-4 py-2 bg-indigo-500 text-base font-medium text-white hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm transition">
                                                        <!---->
                                                        <span>Submit</span>
                                                    </button>
                                                    <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Tutup</button>
...
@push('js-bottom')
    <script>
        let exam_id = {{ $exam->id }}; // Ambil ID ujian dari Blade
        let currentIndex = 0;
        ...
        function loadQuestion(index) {
            $.ajax({
                ...
                success: function(reply) {
                    ...
                    $(".question").text(reply?.question?.question);
                    ...
                    $.each(reply?.question?.answers, function(i, answer) {
                        ...
                        answers += `
                            <div class="flex items-center mb-2">
                                <label for="optionsRadiosa" class="ml-3 w-full flex items-center">
                                    <input name="answer" type="radio" id="optionsRadios${label}" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 mr-2" value="${label}">
